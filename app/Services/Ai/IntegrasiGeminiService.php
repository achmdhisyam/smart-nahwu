...
class IntegrasiGeminiService
{
    ...
    public function __construct(
        PenyusunPromptService $promptBuilder,
        PemformatHasilService $formatter,
        CacheAnalisisService $cache
    ) {
     ...
     public function analyze(string $sentence, array $nlpResult, array $relatedRules, ?int $userId = null): array
     {
        ...
        try {
            ...
            if (isset($formattedResult['word_by_word_analysis']) && is_array($formattedResult['word_by_word_analysis'])) {
                $allRules = \App\Models\KaidahGramatika::all();
                foreach ($formattedResult['word_by_word_analysis'] as $index => &$item) {
                    ...
                    if (isset($item['explanation'])) {
                        foreach ($allRules as $rule) {
                            if (stripos($item['explanation'], $rule->kode_kaidah) !== false) {
                                $item['explanation'] = str_ireplace($rule->kode_kaidah, '"' . $rule->teks_kaidah . '"', $item['explanation']);
                            }
                        }
                    }
                }
    ...
    protected function fallbackToLocalRuleEngine(string $sentence, array $nlpResult, array $relatedRules): array
    {
        ...
        foreach ($tokens as $index => $token) {
            ...
            if ($isParticle) {
                ...
                $rulesOfChapter = \App\Models\KaidahGramatika::where('bab_id', $ref['chapter_id'] ?? 0)->pluck('teks_kaidah')->toArray();
    ...
    protected function isWordVerb(string $text): ?array
    {
        ...
        $commonVerbs = [
            ...
            'خلق', 'ولد', 'كتب', 'سافر', 'ظن', 'حsub', 'خal', 'زعم', 'رأى', 'علم', 
            'وجد', 'اتخذ', 'جعل', 'سمع', 'كان', 'صار', 'أصبح', 'أمسى', 'أضحى', 'ظل', 
            'بات', 'زال', 'فتئ', 'انفك', 'برح', 'دام', 'ليس', 'قال', 'أرad', 'شاء',
        ...
        if (Str::endsWith($gundul, ['ت', 'na', 'wa', 'tem'])) {
            return ['type' => 'Madhi', 'morphology' => 'Fi\'il Madhi (Kata Kerja Lampau)'];
        }

        $gundulLast = mb_substr($gundul, -2);
        if (in_array($gundulLast, ['تا', 'تم', 'تن', 'نا']) || mb_substr($gundul, -1) === 'ت') {
            return ['type' => 'Madhi', 'morphology' => 'Fi\'il Madhi (Kata Kerja Lampau)'];
        }
