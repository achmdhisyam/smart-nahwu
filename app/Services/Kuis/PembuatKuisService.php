...
class PembuatKuisService
{
    /**
     * Menggenerasi kuis pilihan ganda berdasarkan materi bab Jurumiyah menggunakan Gemini AI.
     *
     * @param BabJurumiyah $chapter
     * @return BuatKuis
     */
    public function generate(BabJurumiyah $chapter): BuatKuis
    {
        // 1. Cek apakah kuis untuk bab ini sudah pernah dibuat sebelumnya
        $existingQuiz = BuatKuis::where('bab_id', $chapter->id)->first();
        if ($existingQuiz) {
            return $existingQuiz;
        }

        // 2. Jika belum ada, buat prompt dengan menyertakan teks materi dan contoh dari bab
        $chapterContent = "Bab: {$chapter->judul}\nDefinisi: {$chapter->definisi}\n";
        
        $rulesText = "";
        foreach ($chapter->kaidahGramatika as $rule) {
            $rulesText .= "- {$rule->teks_kaidah}\n";
        }
        
        $examplesText = "";
        foreach ($chapter->contohGramatika as $example) {
            $examplesText .= "- Arab: {$example->teks_arab}, Terjemahan: {$example->terjemahan}\n";
        }

        $prompt = $this->buildQuizPrompt($chapter->judul, $chapterContent . $rulesText . $examplesText);
        ...
        try {
            ...
            return BuatKuis::create([
                'bab_id' => $chapter->id,
                'judul' => 'Latihan Soal: ' . $chapter->judul,
                'data_pertanyaan' => $quizData,
            ]);
  ...
  "questions": [
    {
      ...
      "correct_answer": "A" atau "B" atau "C" or "D",
...
PROMPT;
     ...
     */
    protected function createFallbackQuiz(BabJurumiyah $chapter): BuatKuis
    {
        $mockQuestions = [
            'questions' => [
                [
                    'id' => 1,
                    'question' => "Manakah di bawah ini contoh kalimat yang paling tepat menggambarkan bab: {$chapter->judul}?",
                    'options' => [
        ...
        return BuatKuis::create([
            'bab_id' => $chapter->id,
            'judul' => 'Latihan Soal (Standard): ' . $chapter->judul,
            'data_pertanyaan' => $mockQuestions,
        ]);
    }
}
