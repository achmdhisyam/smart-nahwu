...
class JsonImporterService
{
    ...
    public function import(string $jsonPath): void
    {
        ...
        DB::transaction(function () use ($data) {
            ...
            foreach ($data as $index => $chapterItem) {
                ...
                $chapter = JurumiyahChapter::create([
                    'title' => $chapterItem['chapter'],
                    'definition' => $chapterItem['definition'],
                    'order_num' => $index + 1,
                    'learning_step' => $step,
                ]);
                ...
                foreach ($chapterItem['rules'] as $ruleIndex => $ruleText) {
                    $ruleCode = 'RULE_' . strtoupper(Str::slug($chapter->title, '_')) . '_' . ($ruleIndex + 1);
                ...
                foreach ($chapterItem['examples'] as $exampleItem) {
                    GrammarExample::create([
                        'chapter_id' => $chapter->id,
                        'arabic_text' => $exampleItem['arabic'],
    ...
    protected function clearDatabase(): void
    {
        ...
        ArabicParticle::truncate();
        GrammarExample::truncate();
        GrammarRule::truncate();
        JurumiyahChapter::truncate();
    ...
    protected function extractParticlesFromRule(int $chapterId, string $ruleText): void
    {
        $keywords = [
            ...
            'nashab' => ['amil nashab', 'amil penashab'],
        ...
        if ($matchedType) {
            ...
            if (!empty($words[0])) {
                foreach ($words[0] as $word) {
                    ...
                    $word = trim($word);
                    if (!empty($word)) {
                        ArabicParticle::firstOrCreate([
                            'chapter_id' => $chapterId,
                            'particle_text' => $word,
                            'particle_type' => $matchedType,
                        ]);
                    }
                }
            }
