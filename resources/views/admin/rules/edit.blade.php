...
<div class="max-w-xl mx-auto space-y-6 py-6">
    ...
    <div>
        <h1 class="text-3xl font-extrabold text-[#1b4332]">Edit Kaidah: {{ $rule->rule_code }}</h1>
        ...
        <form action="{{ route('admin.rules.update', $rule->id) }}" method="POST" class="space-y-5">
            ...
            <div class="space-y-1">
                ...
                >
                    @foreach($chapters as $chapter)
                        <option value="{{ $chapter->id }}" {{ $rule->chapter_id == $chapter->id ? 'selected' : '' }}>
                            {{ $chapter->title }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="space-y-1">
                ...
                <textarea 
                    ...
                    required
                >{{ old('rule_text', $rule->rule_text) }}</textarea>
            </div>
