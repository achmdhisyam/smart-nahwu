                ...
                <tbody class="divide-y divide-[#e6dec9] text-[#2b3a32] text-sm">
                    @forelse($rules as $rule)
                        <tr class="hover:bg-[#fbf8f1]/40 transition">
                            <td class="px-6 py-4 font-mono font-bold text-[#b45309] text-xs">{{ $rule->rule_code }}</td>
                            <td class="px-6 py-4 font-bold text-[#1b4332]">{{ $rule->chapter->title ?? '-' }}</td>
                            <td class="px-6 py-4 text-xs text-[#5c6f60] max-w-sm leading-relaxed">{{ $rule->rule_text }}</td>
