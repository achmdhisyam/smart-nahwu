...
<div class="max-w-xl mx-auto space-y-6 py-6">
    ...
    <div>
        <h1 class="text-3xl font-extrabold text-[#1b4332]">Edit Bab: {{ $chapter->title }}</h1>
    ...
    <div class="glass bg-white rounded-3xl p-6 sm:p-8 shadow-sm border border-[#e6dec9]">
        <form action="{{ route('admin.chapters.update', $chapter->id) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-1">
                ...
                <textarea 
                    ...
                    required
                >{{ old('definition', $chapter->definition) }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="space-y-1">
                    ...
                    <input 
                        ...
                        name="order_num" 
                        id="order_num" 
                        value="{{ old('order_num', $chapter->order_num) }}"
                        class="w-full bg-white border border-[#e6dec9] rounded-xl px-4 py-3 text-[#2b3a32] focus:outline-none focus:ring-2 focus:ring-[#1b4332]/30 focus:border-[#1b4332]"
                        required
                    >
                </div>
                ...
                <div class="space-y-1">
                    ...
                    <input 
                        ...
                        name="learning_step" 
                        id="learning_step" 
                        value="{{ old('learning_step', $chapter->learning_step) }}"
                        min="1"
                        max="9"
                        class="w-full bg-white border border-[#e6dec9] rounded-xl px-4 py-3 text-[#2b3a32] focus:outline-none focus:ring-2 focus:ring-[#1b4332]/30 focus:border-[#1b4332]"
                        required
