...
<div class="max-w-5xl mx-auto space-y-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <form action="{{ route('riwayat.index') }}" method="GET" class="w-full sm:w-auto flex items-center space-x-2">
            ...
            @if(!empty($search))
                <a href="{{ route('riwayat.index') }}" class="text-xs text-slate-400 hover:text-slate-200 underline">Reset</a>
            @endif
        </form>
    </div>
    ...
    <div class="glass rounded-3xl p-6 sm:p-8 shadow-xl border border-slate-700/50">
        @if($histories->isEmpty())
            <div class="text-center py-16 space-y-4">
                ...
                <a href="/analisis" class="inline-block bg-gradient-to-r from-teal-500 to-indigo-600 hover:from-teal-400 hover:to-indigo-500 text-white font-bold py-2.5 px-6 rounded-xl text-sm transition shadow-lg shadow-teal-500/20">
                    Mulai Analisis Baru
                </a>
            </div>
        @else
                        ...
                        @foreach($histories as $history)
                            <tr class="hover:bg-slate-800/20 transition duration-150">
                                ...
                                <td class="py-4 px-2 text-right flex items-center justify-end space-x-2">
                                    ...
                                    <!-- Tombol Hapus -->
                                    <form action="{{ route('riwayat.destroy', $history->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus riwayat ini?');">
                                        @csrf
                                        @method('DELETE')
                                        <button 
                                            type="submit" 
